...
def plot(csv_path: str):
    ...
    plt.title(
        "Score distribution of the MCTS 1K agent across UCT policy variations in 3-player games"
    )
    subset = make_subset(
    ...
    sns.heatmap(
        ...
        cmap=sns.cubehelix_palette(
            start=2.6, rot=0, dark=0.1, light=0.95, reverse=True, as_cmap=True
        ),
        annot=True,
        vmin=0,
        vmax=12,
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(PLOT_PATH, "heatmap_policy_player_count_winrate.png"), dpi=300
    )
    plt.clf()
    ...
    def func(x, a, b, c):
        return b + a * np.log(x + np.abs(c))
        # return b + a * x

    param, cov = scipy.optimize.curve_fit(
        func, subset_mean["agent_playouts"], subset_mean["score"], (1, 12, 1)
    )
    ...
    g = sns.lineplot(
        ax=g, x=xs, y=ys, color="#333", alpha=0.7, label="Regression to $\log(x)$"
    )
    ...
    plt.savefig(
        os.path.join(PLOT_PATH, "combined_performance_plot.png"),
        dpi=300,
    )
    plt.clf()
